...
var HOME, VERSION, config, exitWithError, fileExists, fs, i3Path, mkConfig, pathUtil, program, sh, theme, themesAvailable, themesDir, tmpConfigPath, tmpdir, validation, yaml, _ref,
...
program.version(VERSION).usage('<theme> [options]').option('-c, --config <file>', 'The config file the theme should be applied to. Defaults to the default i3 location.').option('-o, --output <file>', 'Applies the theme, attempts to validate the result, and writes it to <file>. Prints to STDOUT if no output file is given.').parse(process.argv);
